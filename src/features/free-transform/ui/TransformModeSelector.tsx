...
export const TransformModeSelector: React.FC<Props> = ({
  ...
  return (
    <Box className={cn('mb-6')}>
      <Typography variant="h6" className={cn('font-bold mb-4 text-gray-800')}>
        변형 모드
      </Typography>
      
      <Box className={cn('grid grid-cols-2 gap-3')}>
        {modes.map(({ value, label, icon, desc }) => (
          ...
          >
            <Box className={cn('flex flex-col items-center text-center')}>
              <Box
                className={cn(
              ...
              <Typography 
                ...
                className={cn(
                  'font-semibold mb-1',
                  transformMode === value ? 'text-blue-800' : 'text-gray-800'
                )}
              >
                {label}
              </Typography>
              <Typography 
                variant="caption" 
                className={cn(transformMode === value ? 'text-blue-600' : 'text-gray-500')}
              >
                {desc}
              </Typography>
            </Box>
            {transformMode === value && (
              <Box className={cn('absolute -top-1 -right-1 w-4 h-4 bg-blue-500 rounded-full border-2 border-white flex items-center justify-center')}>
                <Box className={cn('w-2 h-2 bg-white rounded-full')} />
              </Box>
            )}
          </Paper>
        ))}
